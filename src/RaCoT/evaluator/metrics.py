...
from RaCoT.evaluator.utils import normalize_answer
...
class LLMJudge(BaseMetric):
    ...
    def __init__(self, config):
        ...
        if "llm_judge_setting" in config["metric_setting"]:
            ...
            raise ValueError("No available LLM settings!")
        ...
        if model_path is None:
            raise ValueError("None model path")
    ...
    def extract_judge_score(answer: str, split_str: str = "Total rating:") -> float:
        try:
            ...
            warnings.warn(f"Failed to extract judge score: {e}")
            return 0
    ...
    def calculate_metric(self, data):
        ...
        judge_input_prompt = [
            self.JUDGE_PROMPT.format(question=q, answer=a) for q, a in zip(question_list, pred_list)
        ]
