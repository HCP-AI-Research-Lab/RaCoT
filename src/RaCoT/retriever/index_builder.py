...
from RaCoT.retriever.utils import (
    load_model,
    load_corpus,
    pooling,
...
class Index_Builder:
    ...
    def __init__(
        ...
        self.is_clip = ("clip" in self.retrieval_method) or (
            self.model_path is not None and "clip" in self.model_path
        )
        if not self.is_clip:
            try:
                ...
                pass
        if self.is_clip:
            LOGGER.info("Using CLIP-compatible retrieval model.")
        ...
        LOGGER.info("Index save directory: %s", self.save_dir)
        ...
        LOGGER.info("Finished loading corpus from %s", self.corpus_path)
    ...
    def build_index(self):
        ...
        if self.retrieval_method == "bm25":
            if self.bm25_backend == "pyserini":
                ...
                raise ValueError(f"Invalid bm25 backend: {self.bm25_backend}")
    ...
    def build_seismic_index(self):
        ...
        r"""Full Command:
        python -m RaCoT.retriever.index_builder \
        ...
        if torch.cuda.device_count() > 1:
            LOGGER.info("Using %d GPUs for sparse indexing.", torch.cuda.device_count())
        ...
        output_path = os.path.join(
            self.save_dir, f"{corpus_name }_{self .retrieval_method }_embedded.jsonl"
        )
        ...
        if self.corpus_embedded_path:
            LOGGER.info("Using cached corpus in seismic format.")
            if self.nknn > 0:
            ...
            for doc in self.corpus:
                ...
                if len(batch) >= self.batch_size:
                    ...
                    if processed_docs % 100 == 0 or (current_time - last_update) > 5.0:
                        ...
                        progress_bar.update(
                            100 if processed_docs % 100 == 0 else processed_docs % 100
                        )
    ...
    def _get_sparse_embedding(self, texts: List[str], model, tokenizer) -> List[Dict[str, float]]:
        ...
        with torch.no_grad():
            ...
            v_repr = torch.max(scores, dim=1)[0]
            ...
            token_id_to_token = {
                ...
                for idx, tok in zip(
                    unique_token_ids,
                    tokenizer.convert_ids_to_tokens(unique_token_ids.tolist()),
                )
            }
    ...
    def build_bm25_index_pyserini(self):
        ...
        LOGGER.info("Start building BM25 index with pyserini backend.")
        pyserini_args = [
            "--collection",
            "JsonCollection",
            "--input",
        ...
        if zh_flag:
            LOGGER.info("Detected Chinese corpus; enabling zh analyzer for BM25.")
        ...
        LOGGER.info("Finished building BM25 index (pyserini).")
    ...
    def build_bm25_index_bm25s(self):
        ...
        LOGGER.info("Finished building BM25 index (bm25s).")
    ...
    def encode_all(self):
        ...
        if self.gpu_num > 1:
            LOGGER.info("Using multi-GPU dense encoding (%d GPUs).", self.gpu_num)
            self.batch_size = self.batch_size * self.gpu_num
            all_embeddings = self.encoder.multi_gpu_encode(
                encode_data, batch_size=self.batch_size, is_query=False
            )
        else:
    ...
    def encode_all_clip(self):
        ...
        for modal, _ in modal_dict.items():
            ...
            if self.gpu_num > 1:
                LOGGER.info(
                    "Using multi-GPU CLIP encoding for %s modal (%d GPUs).",
                    modal,
                    self.gpu_num,
                )
                self.batch_size = self.batch_size * self.gpu_num
                all_embeddings = self.encoder.multi_gpu_encode(
                    encode_data, batch_size=self.batch_size, modal=modal
                )
            else:
                all_embeddings = self.encoder.encode(
                    encode_data, batch_size=self.batch_size, modal=modal
    ...
    def build_dense_index(self):
        ...
        if self.is_clip:
            from RaCoT.retriever.encoder import ClipEncoder
            ...
            from RaCoT.retriever.encoder import STEncoder
            ...
            from RaCoT.retriever.encoder import Encoder
        ...
        if self.is_clip:
            ...
            if os.path.exists(self.index_save_path):
                LOGGER.warning("The index file already exists and will be overwritten.")
            self.save_faiss_index(all_embeddings, self.faiss_type, self.index_save_path)
        LOGGER.info("Finished building dense index.")
    ...
    def save_faiss_index(
        ...
        LOGGER.info("Creating FAISS index: %s", index_save_path)
