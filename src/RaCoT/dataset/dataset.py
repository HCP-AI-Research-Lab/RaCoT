...
class Item:
    ...
    _IMMUTABLE_KEYS = {"id", "question", "golden_answers", "output", "choices"}
    ...
    def __getattr__(self, attr_name: str) -> Any:
        if attr_name in self._CORE_ATTRS:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        """
        from RaCoT.dataset.utils import (
            convert_numpy,
            remove_images,
            clean_prompt_image,
        )
        ...
        output_payload = dict(self.output)
        if "prompt" in output_payload:
            output_payload["prompt"] = clean_prompt_image(output_payload["prompt"])

        output["output"] = remove_images(convert_numpy(output_payload))
        if self.metadata:
...
class Dataset:
    ...
    def __init__(
        ...
        if data is None:
            ...
            self.data = self._build_items_from_data(data)

    def _build_items_from_data(self, data: List[Any]) -> List[Item]:
        LOGGER.info("Loading dataset from provided in-memory data.")
        if len(data) == 0:
            return []
        if all(isinstance(item, dict) for item in data):
            return [Item(item_dict) for item_dict in data]
        if all(isinstance(item, Item) for item in data):
            return data
        raise TypeError("`data` must be a list of dicts or Item objects.")

    @staticmethod
    def _load_jsonl_items(dataset_path: str) -> List[Item]:
        items: List[Item] = []
    ...
    @staticmethod
    def _load_json_items(dataset_path: str) -> List[Item]:
        with open(dataset_path, "r", encoding="utf-8") as f:
            payload = json.load(f)
        if isinstance(payload, dict):
            payload = [payload]
        if not isinstance(payload, list):
            raise ValueError("JSON dataset must be a dict item or a list of items.")
        return [Item(item_dict) for item_dict in payload]

    @staticmethod
    def _load_parquet_items(dataset_path: str) -> List[Item]:
        items: List[Item] = []
    ...
    def _apply_sampling(self, data: List[Item]) -> List[Item]:
        ...
        if self.sample_num > len(data):
            warnings.warn(
                f"`sample_num` ({self.sample_num}) exceeds dataset size ({len(data)}); using full dataset."
            )
            self.sample_num = len(data)

        if self.random_sample:
            LOGGER.info("Randomly sampling %d items from dataset.", self.sample_num)
            return random.sample(data, self.sample_num)
        return data[: self.sample_num]

    def _load_data(self, dataset_path: Optional[str]) -> List[Item]:
        """Load data from the provided dataset_path."""
        if not dataset_path:
            raise ValueError("`dataset_path` must be provided when `data` is None.")
        if not os.path.exists(dataset_path):
            raise FileNotFoundError(f"Dataset file {dataset_path} not found.")

        if dataset_path.endswith(".jsonl"):
            data = self._load_jsonl_items(dataset_path)
        elif dataset_path.endswith(".json"):
            data = self._load_json_items(dataset_path)
        elif dataset_path.endswith(".parquet"):
            data = self._load_parquet_items(dataset_path)
        else:
            raise NotImplementedError(f"Unsupported dataset format: {dataset_path}")

        return self._apply_sampling(data)
