    ...
    def _load_file_config(self, config_file_path: str):
        ...
        if config_file_path:
            with open(config_file_path, "r", encoding="utf-8") as f:
                loaded = yaml.load(f.read(), Loader=self.yaml_loader) or {}
    ...
    def _set_additional_key(self):
        ...
        def set_retrieval_keys(model2path, model2pooling, method2index, config):
            ...
            if config["index_path"] is None:
                config["index_path"] = method2index.get(retrieval_method)
                if config["index_path"] is None:
                    warnings.warn(
                        f"Index path for retrieval method `{retrieval_method}` is not configured."
                    )
            ...
            if config.get("retrieval_pooling_method") is None:
                config["retrieval_pooling_method"] = set_pooling_method(
                    retrieval_method, model2pooling
                )
            ...
            if config.get("rerank_model_path", None) is None:
                if rerank_model_name is not None:
                    config["rerank_model_path"] = model2path.get(
                        rerank_model_name, rerank_model_name
                    )
            if config.get("rerank_pooling_method", None) is None:
                if rerank_model_name is not None:
                    config["rerank_pooling_method"] = set_pooling_method(
                        rerank_model_name, model2pooling
                    )
            return config
        ...
        self.final_config = set_retrieval_keys(
            model2path, model2pooling, method2index, self.final_config
        )
        if "multi_retriever_setting" in self.final_config:
            ...
            allowed_merge_methods = {"concat", "rrf", "rerank", None}
            if multi_retriever_config["merge_method"] not in allowed_merge_methods:
                raise ValueError(
                    f"Invalid merge_method: {multi_retriever_config['merge_method']}. "
                    f"Expected one of {allowed_merge_methods}."
                )
            if multi_retriever_config["merge_method"] == "rerank":
                ...
                if rerank_model_name is None:
                    raise ValueError(
                        "`multi_retriever_setting.rerank_model_name` is required when merge_method='rerank'."
                    )
                multi_retriever_config["rerank_max_length"] = multi_retriever_config.get(
                    "rerank_max_length", 512
                )
                multi_retriever_config["rerank_batch_size"] = multi_retriever_config.get(
                    "rerank_batch_size", 256
                )
                multi_retriever_config["rerank_use_fp16"] = multi_retriever_config.get(
                    "rerank_use_fp16", True
                )

                if multi_retriever_config.get("rerank_model_path", None) is None:
                    if rerank_model_name is not None:
                        multi_retriever_config["rerank_model_path"] = model2path.get(
                            rerank_model_name, rerank_model_name
                        )
                if multi_retriever_config.get("rerank_pooling_method", None) is None:
                    if rerank_model_name is not None:
                        multi_retriever_config["rerank_pooling_method"] = set_pooling_method(
                            rerank_model_name, model2pooling
                        )
            ...
            for retriever_config in retriever_config_list:
                ...
                retriever_config = set_retrieval_keys(
                    model2path, model2pooling, method2index, retriever_config
                )
        ...
        if self.final_config.get("generator_model_path") is None:
            self.final_config["generator_model_path"] = model2path.get(
        ...
        if "refiner_name" in self.final_config:
            ...
            if (
                "refiner_model_path" not in self.final_config
                or self.final_config["refiner_model_path"] is None
            ):
        ...
        from RaCoT.utils.constants import OPENAI_MODEL_DICT
