...
from RaCoT.generator.utils import (
...
class InternVL2InferenceEngine(BaseInferenceEngine):
    ...
    def generate(self, input_list, **params):
        ...
        final_image_list = [
            [
                self.load_image(img, max_num=12).to(self.model.dtype).to(self.model.device)
                for img in image_list
            ]
            for image_list in final_image_list
        ]
    ...
    def generate(self, input_list, **params):
        ...
        for messages in input_list:
            ...
            for message in messages:
                ...
                if isinstance(message["content"], list):
                    image_list = [
                        item["image"] for item in message["content"] if item["type"] == "image"
                    ]
        ...
        texts = self.tokenizer.apply_chat_template(
            new_input_list, tokenize=False, add_generation_prompt=True
        )
        inputs = self.processor(
            text=texts,
            images=visual_list,
...
class HFModelInferenceEngineFactory:
    ...
    def get_engine(model_path, device="cpu", **kwargs):
        ...
        for (
            engine_name,
            engine_class,
        ) in HFModelInferenceEngineFactory._engine_map.items():
...
class HFMultiModalGenerator(BaseMultiModalGenerator):
    ...
    def generate(self, input_list: list, batch_size=None, **params):
        ...
        for idx in trange(0, len(input_list), batch_size, desc="Generation process: "):
            ...
            output_responses.extend(
                self.inference_engine.generate(batch_prompts, **generation_params)
            )
