...
from RaCoT.generator.utils import resolve_max_tokens
...
class EncoderDecoderGenerator(BaseGenerator):
    ...
    def __init__(self, config):
        ...
        if "t5" in arch or "fusionindecoder" in arch:
            if self.fid:
                from RaCoT.generator.fid import FiDT5
            ...
            if self.fid:
                raise ValueError(
                    "Unsupported FiD configuration: `use_fid=True` requires a T5/FusionInDecoder architecture."
                )
    ...
    def generate(self, input_list: List, batch_size=None, **params):
        ...
        if "stop" in generation_params:
            from RaCoT.generator.stop_word_criteria import StopWordCriteria
...
class VLLMGenerator(BaseGenerator):
    ...
    def generate(
        ...
        **params,
        ...
        generation_params = resolve_max_tokens(
            params, generation_params, prioritize_new_tokens=False
        )
        ...
        if return_raw_output:
            ...
            generated_texts = [
                (
                    [c.text for c in output.outputs]
                    if len(output.outputs) > 1
                    else output.outputs[0].text
                )
        ...
        **params,
        ...
        for idx in trange(0, len(input_list), batch_size, desc="Generation process: "):
            with torch.inference_mode():
                ...
                gen_score = (
                    torch.gather(logits, 2, generated_ids[:, :, None]).squeeze(-1).cpu().tolist()
                )
    ...
    def _load_model(self, model=None):
        ...
        if model is None:
            ...
            if self.gpu_num > 1:
                ...
                max_gpu_memory = (
                    str(int(min(available_gpu_memory) * gpu_memory_utilization)) + "GiB"
                )
