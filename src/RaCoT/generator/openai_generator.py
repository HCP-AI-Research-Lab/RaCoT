...
class OpenaiGenerator:
    ...
    def __init__(self, config):
        ...
        try:
            ...
            warnings.warn(
                "This model is not supported by tiktoken. "
                f"Falling back to gpt-3.5-turbo tokenizer. ({e})"
            )
    ...
    async def _generate_async(
        ...
        for res in results:
            if mode == "chat":
                ...
                text = res.text
            if "stop" in generation_params and not any(
                [text.endswith(stop_str) for stop_str in generation_params["stop"]]
            ):
    ...
    def generate(
        ...
        future = asyncio.run_coroutine_threadsafe(
            self._generate_async(
                input_list, batch_size=batch_size, return_scores=return_scores, **params
            ),
            loop,
        )
