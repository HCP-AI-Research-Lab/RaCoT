...
from RaCoT.refiner import BaseRefiner
from RaCoT.prompt import PromptTemplate
from RaCoT.retriever.encoder import Encoder, STEncoder
from RaCoT.utils import hash_object
...
class KGTraceRefiner(BaseRefiner):
    def __init__(self, config, retriever=None, generator=None):
        ...
        if retriever is None:
            LOGGER.info("Loading retriever from config.")
            from RaCoT.utils import get_retriever

            self.retriever = get_retriever(config)
        else:
            self.retriever = retriever
        if generator is None:
            LOGGER.info("Loading generator from config.")
            from RaCoT.utils import get_generator
        ...
        if config["dataset_name"].lower() == "hotpotqa":
            from RaCoT.prompt.trace_examplars import (
        ...
        triple_examplars_text_list = [
            f"title: {item ['title']} text: {item ['text']}" for item in self.triple_examplars
        ]
        self.triple_examplars_embeddings = self.encoder.encode(
            triple_examplars_text_list, is_query=False
        )
        ...
        self.chain_examplars_embeddings = self.encoder.encode(
            chain_examplars_text_list, is_query=True
        )
    ...
    def parse_triple_output(self, doc_list, output_list):
        def parse_model_output(triples_text: str):
            ...
            for one_triple_text in re.findall(r"<([^>]*)>", triples_text):
                ...
                if len(pieces) != 3:
                    LOGGER.warning(
                        'Invalid triple format encountered: "%s". Skipping this triple.',
                        one_triple_text,
                    )
                    continue
    ...
    def extract_document_triples(self, queries, retrieval_results):
        ...
        if len(docs_content) > 0:
            ...
            prompt_template = PromptTemplate(
                config=self.config, system_prompt=system_prompt, user_prompt=user_prompt
            )
            prompts = [
    ...
    def build_prompt_for_reasoning_chain(
        ...
        if use_demonstration and len(generating_chain_examplars) > 0:
            ...
            for i, (rp_examplar, grp_examplar) in enumerate(
                zip(final_chain_examplars, generating_chain_examplars)
            ):
    ...
    def get_answer_token_indices(self, tokenizer, num_choices, token_ids):
        ...
        if self.token_id_to_choice_map is None:
            ...
            for choice in choices:
                ...
                self.token_id_to_choice_map[
                    tokenizer.encode(" {}".format(choice), add_special_tokens=False)[-1]
                ] = choice

        answer_token_indices = torch.zeros((token_ids.shape[0],), dtype=token_ids.dtype).fill_(
            token_ids.shape[1] - 1
        )
    ...
    def get_reasoning_chain(self, all_query, all_doc_triples, triple_to_doc_ids):
        all_generating_chain_examplars, all_final_chain_examplars = (
            self.get_examplars_for_reasoning_chain(all_query)
        )
        all_chain_results = []
        ...
        for (
            ...
            zip(
                all_query,
                all_doc_triples,
                all_generating_chain_examplars,
                all_final_chain_examplars,
                triple_to_doc_ids,
            ),
            total=len(all_query),
            desc="Generating reasoning chain for query",
        ):
            flatten_triples = sum(doc_triples, [])
            flatten_doc_ids = sum(
                [
            ...
            triple_text = [
                "<{}; {}; {}>".format(
                    triple_item["head"], triple_item["relation"], triple_item["tail"]
                )
            ...
            for j in range(self.max_chain_length):
                ...
                path_query_embeddings = torch.tensor(
                    self.encoder.encode(path_queries, is_query=True)
                )
                ...
                path_triples_similarities = path_triples_similarities - 10000 * (
                    1.0 - candidate_triples_mask
                )
                topk_most_relevant_triples_indices = torch.topk(
                    path_triples_similarities,
                    k=min(self.topk_triple_select, num_total_triples),
                    dim=1,
                )[1].tolist()

                input_prompts = []
                exisiting_triples = [[triple_text[idx] for idx in path] for path in paths]
                candidate_triples = [
                ...
                generate_output = self.generator.generate(
                    input_prompts, max_tokens=32, return_dict=True
                )
                ...
                answer_token_logits = generated_token_logits.gather(
                    1,
                    answer_token_indices[:, None, None].expand(
                        -1, -1, generated_token_logits.shape[-1]
                    ),
                )
                ...
                answer_token_probs = F.softmax(
                    answer_token_logits[:, choices_token_ids_list], dim=1
                )
                ...
                for i in range(len(paths)):
                    ...
                    if torch.all(torch.isnan(topk_choices_probs[i])):
                        LOGGER.warning(
                            "No valid choice in generated results. Generated text: %s",
                    ...
                    for b in range(self.num_beams):
                        ...
                        if current_choice != "A" and (
                            ord(current_choice) - ord("B")
                            >= len(topk_most_relevant_triples_indices[i])
                        ):
                            continue
                        ...
                        if current_choice == "A":
                            ...
                            new_paths.append(
                                paths[i]
                                + [
                                    topk_most_relevant_triples_indices[i][
                                        ord(current_choice) - ord("B")
                                    ]
                                ]
                            )
                ...
                if len(new_paths) != len(new_paths_scores) or len(new_paths) != len(
                    new_paths_finished
                ):
                    raise RuntimeError(
                        "Reasoning-path construction produced inconsistent path/score/state lengths."
                    )
                new_paths_sorted_indices = sorted(
                    range(len(new_paths_scores)),
                    key=lambda x: new_paths_scores[x],
                    reverse=True,
                )
    ...
    def batch_run(self, dataset):
        ...
        LOGGER.info("Begin extracting triples.")
        all_doc_triples = self.extract_document_triples(all_query, retrieval_results)
        triple_to_doc_ids = [
            [doc_item["id"] for doc_item in query_retrieval_result]
            for query_retrieval_result in retrieval_results
        ]
        ...
        LOGGER.info("Begin generating reasoning chain.")
        reasoning_chain_result = self.get_reasoning_chain(
            all_query, all_doc_triples, triple_to_doc_ids
        )
        dataset.update_output("reasoning_chain", reasoning_chain_result)
        LOGGER.info("Finished generating reasoning chain.")
        ...
        if self.context_type == "triples":
            ...
            for query_chain_result, query_retrieval_results in zip(
                ...
                for chain in query_chain_result:
                    for _, doc_id in zip(chain["triples"], chain["triple_doc_ids"]):
                        chains_doc_id_count_dict[doc_id] = (
                            chains_doc_id_count_dict.get(doc_id, 0) + 1
                        )

                ranked_chains_doc_id = sorted(
                    chains_doc_id_count_dict.items(), key=lambda x: x[1], reverse=True
                )
                query_doc_to_idx = {
                    doc_item["id"]: idx for idx, doc_item in enumerate(query_retrieval_results)
                }
            ...
            raise ValueError(
                f"Unsupported context_type: {self.context_type!r}. Expected 'triples' or 'triple-doc'."
            )
