...
class BaseRefiner:
    ...
    def __init__(self, config):
        ...
        self.input_prompt_flag = (
            config["refiner_input_prompt_flag"] if "refiner_input_prompt_flag" in config else False
        )
...
class LLMLinguaRefiner(BaseRefiner):
    ...
    def __init__(self, config):
        ...
        from RaCoT.refiner.llmlingua_compressor import PromptCompressor
...
class SelectiveContextRefiner(BaseRefiner):
    ...
    def __init__(self, config):
        super().__init__(config)
        from RaCoT.refiner.selective_context_compressor import SelectiveContext
...
class ExtractiveRefiner(BaseRefiner):
    ...
    def __init__(self, config):
        ...
        self.mini_batch_size = (
            config["refiner_mini_batch_size"] if "refiner_mini_batch_size" in config else 256
        )
    ...
    def batch_run(self, dataset, batch_size=16):
        ...
        sent_lists = [
            [
                i.strip()
                for i in re.split(r"(?<![A-Za-z]\.)(?<=[.!?])\s+", " ".join(res))
                if len(i.strip()) > 5
            ]
        ...
        for idx in tqdm(range(0, len(questions), batch_size), desc="Refining process: "):
            ...
            sent_embs = []
            for s_index in tqdm(
                range(0, len(flatten_batch_sents), self.mini_batch_size),
                desc="Sentence encoding..,",
            ):
        ...
        for sent_scores, sent_list in zip(score_lists, sent_lists):
            ...
            topk_idxs = torch.topk(
                torch.Tensor(sent_scores), min(self.topk, len(sent_scores))
            ).indices.tolist()
            retain_lists.append(
                [sent_list[idx] for idx in sorted(topk_idxs) if idx < len(sent_list)]
            )
