...
class SelectiveContext:
    ...
    def _prepare_model(self):
        ...
        if self.model_type == "gpt2":
            ...
            LOGGER.info("SelectiveContext language model loaded.")
    ...
    def _get_self_info_via_curie(self, text: str) -> Tuple[List[str], List[float]]:
        ...
        for _ in range(num_retry):
            try:
                ...
                last_error = e
                LOGGER.warning("Curie self-information request failed: %s", e)
                time.sleep(1)

        if response is None:
            raise RuntimeError("Failed to fetch self-information from Curie API.") from last_error
    ...
    def self_info_mask(self, sents: List[str], self_info: List[float], mask_level):
        ...
        masked_context = (
            " ".join(sents_after_mask) if mask_level == "sent" else "".join(sents_after_mask)
        )
    ...
    def __call__(
        self, text: str, reduce_ratio: float = 0.5, reduce_level: str = "phrase"
    ) -> List[str]:
