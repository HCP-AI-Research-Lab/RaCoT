...
def precess_jsonKVpair(k, v, value_type, rate):
    ...
    if value_type == "str" or value_type == "string":
        v = str(v)
        new_v = (
            f"</llmlingua><llmlingua, rate={rate }>" + v + "</llmlingua><llmlingua, compress=False>"
        )
        return (
...
class PromptCompressor:
    ...
    Example:
        ...
        This prints the compressed version of the context.
    ...
    def compress_prompt(
        ...
        if use_token_level_filter:
            ...
            compressed_prompt = (
                self.tokenizer.batch_decode(context[0])[0].replace("<s> ", "").replace("<s>", "")
            )
        else:
            if condition_flag:
                context = context[1:]
    ...
    def get_structured_dynamic_compression_ratio(
        ...
        text_input_ids = self.tokenizer("\n\n".join(context), add_special_tokens=False).input_ids[
            start:
        ]
    ...
    def get_compressed_input(
        ...
        if need_idx.shape[0] < input_ids.shape[1]:
            need_idx = torch.cat(
                [
                    need_idx,
                    torch.ones(input_ids.shape[1] - need_idx.shape[0], dtype=torch.bool).to(
                        need_idx.device
                    ),
                ]
            )
        ...
        if self_loss is not None:
            ...
            self_compressed_attention_mask = self_attention_mask[self_attention_mask == 1][
                need_idx[start:]
            ].unsqueeze(0)
        else:
    ...
    def recover(
        ...
        def match_from_compressed(response_word):
            response_input_ids = self.tokenizer(response_word, add_special_tokens=False)[
                "input_ids"
            ]
    ...
    def segment_structured_context(
        ...
        for text in context:
            ...
            segs_compress = [
                (match[1] == "True" if match[1] else (match[3] == "True" if match[3] else None))
                for match in matches
            ]
